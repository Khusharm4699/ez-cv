...
| **A new kind of HTML resume** | A scrolling, terminal-themed page **and** an AI-assistant chat-UI one-pager — both self-contained single files. |
...
| **Themable** | One accent color in `profile.json` recolors the whole terminal theme and the PDF. |
| **Token-light** | Rendering is plain Python — no model calls. The AI is only used to help you *write*, never to re-render. |

## What you get

Two HTML styles (view the live examples):

- **[Terminal resume](https://khusharm4699.github.io/terminal.html)** — a scrolling dev-console aesthetic.
- **[AI-assistant resume](https://khusharm4699.github.io/ai.html)** — your story as a chat with an AI, with tool-call cards.
- **[PDF](https://khusharm4699.github.io/khush_sharma_resume.pdf)** — the ATS/print version.
...
```
  ...
  scaffold.py  →  a personal resume skill that is yours to keep
        │
        ▼
  Renderers  →  HTML · AI-HTML · Markdown · LaTeX · PDF   (full + one-page)
        │
        ▼
...
cd template
python3 scripts/build_html.py        # -> output/maya_iyer_resume.html
python3 scripts/build_text.py        # -> .md + .tex
python3 scripts/build_html_ai.py     # -> AI-themed one-pager
...
│   ├── scripts/             # build_html, build_html_ai, build_text, _condense,
│   │                        #   _emphasis, _identity, humanize_scan, keyword_audit
